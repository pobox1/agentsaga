...
function ConfiguredReceipt({ registry, workflow }: { registry: Address; workflow: Address }) {
  ...
  return (
    <article className="receipt-card">
      ...
      <div className="receipt-masks"><div><span>Completed</span><code>0x{receipt.completedMask.toString(16).padStart(4, "0")}</code></div><div><span>Failed</span><code>0x{receipt.failedMask.toString(16).padStart(4, "0")}</code></div><div><span>Compensated</span><code>0x{receipt.compensatedMask.toString(16).padStart(4, "0")}</code></div></div>
      <div className="proof-fields"><Proof label="Evidence root" value={receipt.finalEvidenceRoot} /><Proof label="Execution trace" value={receipt.executionTraceHash} /><Proof label="Specification" value={receipt.workflowSpecificationHash} /></div>
      <div className="receipt-footer"><span>Created block {receipt.createdBlock.toString()}</span><span>Finalized block {receipt.finalizedBlock.toString()}</span><a href={`https://testnet.arcscan.app/address/${workflow}`} target="_blank" rel="noreferrer">Verify on ArcScan ↗</a></div>
    </article>
  );
}
